...
def build_layer(f1, f2, N_layer=0, stride=1, skip=True):
    if stride ==1:
        ...
        layers = [Conv3d_ABN(f1, f2, kernel_size=(4, 1, 1), stride=(stride, 1, 1), padding=(1, 0, 0)), \
                  Conv3d_ABN(f2, f2, kernel_size=(1, 4, 1), stride=(1, stride, 1), padding=(0, 1, 0)), \
                  Conv3d_ABN(f2, f2, kernel_size=(1, 1, 4), stride=(1, 1, stride), padding=(0, 0, 1))]
    else:
        layers = [Conv3d_ABN(f1, f2, kernel_size=3, stride=stride, padding=1)]
...
class fpn_big_v2_sync(fpn_big_v2):
