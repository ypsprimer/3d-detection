...
class unet_type_v4(nn.Module):
    def __init__(self, n_inp = 1, feats = [32, 64, 64, 128, 128, 256, 256], blocks=[2,2,2,2,2,2,2], n_pred_p=[1,1,1], n_pred_d=[4,4,4], L_output = [0,1,2],abn = 1, dropout_ratio=0.0):
        ...
        for i in range(6):
            if i==0:
                ...
                setattr(self, 'rnnlz'+str(i), nn.LSTM(feats[i], feats[i] // 2, 1, bidirectional=True))
                setattr(self, 'rnnlz_bn'+str(i), abnblock(feats[i]))
                setattr(self, 'rnnlx'+str(i), nn.LSTM(feats[i], feats[i] // 2, 1, bidirectional=True))
                setattr(self, 'rnnlx_bn'+str(i), abnblock(feats[i]))
                setattr(self, 'rnnly'+str(i), nn.LSTM(feats[i], feats[i] // 2, 1, bidirectional=True))
    ...
    def forward(self, x):
        ...
        x1, _ = self.rnnlz1(x1)
        #x1 = self.rnnlz_bn1(x1)
        x1 = x1.reshape((z_size, bs, x_size, y_size, feat_size))    #(z, bs, x, y, feat)
        x1 = x1.transpose(0, 2)    #(x, bs, z, y, feat)
        x1 = x1.reshape((x_size, -1, feat_size))     #(x, bs*z*y, feat)
        x1, _ = self.rnnlx1(x1)
        #x1 = self.rnnlx_bn1(x1)
        x1 = x1.reshape((x_size, bs, z_size, y_size, feat_size))     #(x, bs, z, y, feat)
        x1 = x1.transpose(0, 3)    #(y, bs, z, x, feat)
        x1 = x1.reshape((y_size, -1, feat_size))     #(y, bs*z*x, feat)
        x1, _ = self.rnnly1(x1)
        #x1 = self.rnnly_bn1(x1)
        x1 = x1.reshape((y_size, bs, z_size, x_size, feat_size))    #(y, bs, z, x, feat)
        x1 = x1.transpose(1, 4)     #(y, feat, z, x, bs)
        x1 = x1.transpose(0, 4)     #(bs, feat, z, x, y)
        x1 = x1 + x1_
        ...
        x2, _ = self.rnnlz2(x2)
        #x2 = self.rnnlz_bn2(x2)
        x2 = x2.reshape((z_size, bs, x_size, y_size, feat_size))    #(z, bs, x, y, feat)
        x2 = x2.transpose(0, 2)    #(x, bs, z, y, feat)
        x2 = x2.reshape((x_size, -1, feat_size))     #(x, bs*z*y, feat)
        x2, _ = self.rnnlx2(x2)
        #x2 = self.rnnlx_bn2(x2)
        x2 = x2.reshape((x_size, bs, z_size, y_size, feat_size))     #(x, bs, z, y, feat)
        x2 = x2.transpose(0, 3)    #(y, bs, z, x, feat)
        x2 = x2.reshape((y_size, -1, feat_size))     #(y, bs*z*x, feat)
        x2, _ = self.rnnly2(x2)
        #x2 = self.rnnly_bn2(x2)
        x2 = x2.reshape((y_size, bs, z_size, x_size, feat_size))    #(y, bs, z, x, feat)
        x2 = x2.transpose(1, 4)     #(y, feat, z, x, bs)
        x2 = x2.transpose(0, 4)     #(bs, feat, z, x, y)
        x2 = x2 + x2_
        ...
        x3, _ = self.rnnlz3(x3)
        #x3 = self.rnnlz_bn3(x3)
        x3 = x3.reshape((z_size, bs, x_size, y_size, feat_size))    #(z, bs, x, y, feat)
        x3 = x3.transpose(0, 2)    #(x, bs, z, y, feat)
        x3 = x3.reshape((x_size, -1, feat_size))     #(x, bs*z*y, feat)
        x3, _ = self.rnnlx3(x3)
        #x3 = self.rnnlx_bn3(x3)
        x3 = x3.reshape((x_size, bs, z_size, y_size, feat_size))     #(x, bs, z, y, feat)
        x3 = x3.transpose(0, 3)    #(y, bs, z, x, feat)
        x3 = x3.reshape((y_size, -1, feat_size))     #(y, bs*z*x, feat)
        x3, _ = self.rnnly3(x3)
        #x3 = self.rnnly_bn3(x3)
        x3 = x3.reshape((y_size, bs, z_size, x_size, feat_size))    #(y, bs, z, x, feat)
        x3 = x3.transpose(1, 4)     #(y, feat, z, x, bs)
        x3 = x3.transpose(0, 4)     #(bs, feat, z, x, y)
        x3 = x3 + x3_
...
class fpn_big_v4(unet_type_v4):
    def __init__(self, config, abn=1, n_inp=1):
        ...
        super().__init__(n_inp = n_inp, feats = [24, 32, 64, 64, 128, 128, 128], abn = abn, n_pred_p= n_pred_p, n_pred_d=n_pred_d, L_output=L_output, dropout_ratio=dropout_ratio)

class fpn_big_v4_sync(fpn_big_v4):
