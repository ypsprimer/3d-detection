...
class unet_type6(nn.Module):
    ...
    def forward(self, x):
        ...
        if 4 in self.L_output:
            ...
            x4p = self.drop_out(x4p)
            x4d = self.before_out3d(x4)
            x4d = self.drop_out(x4d)
        ...
        if 3 in self.L_output:
            ...
            x3p = self.drop_out(x3p)
            x3d = self.before_out3d(x3)
            x3d = self.drop_out(x3d)
        ...
        if 2 in self.L_output:
            ...
            x2p = self.drop_out(x2p)
            x2d = self.before_out2d(x2)
            x2d = self.drop_out(x2d)
        ...
        if 1 in self.L_output:
            ...
            x1p = self.drop_out(x1p)
            x1d = self.before_out1d(x1)
            x1d = self.drop_out(x1d)
        ...
        if 0 in self.L_output:
            ...
            x0p = self.drop_out(x0p)
            x0d = self.before_out0d(x0)
            x0d = self.drop_out(x0d)
...
class fpn_big_type3(unet_type6):
...
class fpn_big_type3_sync(fpn_big_type3):
