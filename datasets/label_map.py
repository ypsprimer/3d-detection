...
class Label_mapping():
    ...
    def quantize(self, iou, omit, train_flag, cls=0, iou_neg_idx = 0):
        # 将iou 量化
        ...
        if not self.max_pos:
            if not omit:
                ...
                results_coord[iou>= self.iou_pos] = train_flag
                results_prob[iou>= self.iou_pos] = cls + 1
    ...
    def generate_one_map(self, bboxs, target_shape, anchor, stride, gt_prob, gt_coord_only, gt_diff, i_anchor):
        ...
        for box in bboxs:
            ...
            if in_x and in_y and in_z:
                ...
                if self.cls_type=='None':
                    new_clip, new_clip_coord = self.quantize(iou, omit, box[6], iou_neg_idx=iou_neg_idx)
                elif self.cls_type=='ssd':
                    new_clip, new_clip_coord = self.quantize(iou, omit, box[6], cls=box[4], iou_neg_idx=iou_neg_idx)
    ...
    def __call__(self, bboxs):
        ...
        for stride, anchors in zip(self.stride_levels, self.anchors):
...
#             if stride
            ...
            for i_anchor, anchor in enumerate(anchors):
                self.generate_one_map(bboxs, target_shape, anchor, stride, gt_prob, gt_coord_only, gt_diff, i_anchor)
        ...
        return gt_prob_fpn, gt_coord_prob_fpn, gt_coord_diff_fpn, gt_diff_fpn
